...
namespace thread {
    ...
    public:
        ...
        auto operator<=>(const CancelableTask& other) const {
            // 首先比较值
            if (auto cmp = runtime <=> other.runtime; cmp != 0) {
                return cmp;
            }
            return this <=> &other;
        }
    ...
    class TaskPack {
    public:
        TaskPack(std::shared_ptr<CancelableTask>&& cancelableTask, std::future<T>&& future)
            : cancelableTask(std::move(cancelableTask)), future(std::move(future)) {
        }

    public:
        std::shared_ptr<CancelableTask> cancelableTask;
        std::future<T> future;
    };
    ...
    public:
        ...
        decltype(auto) addDelayTask(const std::chrono::system_clock::duration& duration, F && f, Args && ... args) {
            ...
            auto cancelableTask = TaskPack(std::make_shared<CancelableTask>([task]() {
                    (*task)();
                }, futureTime), task->get_future());
            addDelayTaskImpl(cancelableTask.cancelableTask);
